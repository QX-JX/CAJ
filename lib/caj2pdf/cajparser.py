...
class CAJParser(object):
    ...
    def _convert_caj(self, dest):
        ...
        # Use PyMuPDF to repair xref (替代 mutool)
        try:
            import fitz
            doc = fitz.open("pdf.tmp")
            doc.save("pdf_toc.pdf", garbage=4, deflate=True)
            doc.close()
        except ImportError:
            # 如果没有 PyMuPDF，尝试使用 mutool
            try:
                ...
                raise SystemExit("需要安装 PyMuPDF (pip install pymupdf) 或 mutool")
